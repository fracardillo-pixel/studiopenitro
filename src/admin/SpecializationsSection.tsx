...
const SpecializationsSection: React.FC = () => {
  ...
  const filteredSpecs = state.specializations.filter((spec) =>
    spec.name.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const handleToggle = (id: string) => {
    dispatch({ type: 'TOGGLE_SPECIALIZATION', payload: id });
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingSpec) {
      ...
      dispatch({ type: 'ADD_SPECIALIZATION', payload: newSpec });
    }
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredSpecs.map((spec) => (
                <tr key={spec.id} className={`hover:bg-gray-50 ${!spec.isActive ? 'opacity-60' : ''}`}>
                    ...
                    <div className="flex justify-center">
                      <button
                        ...
                        className={`relative w-12 h-6 rounded-full transition-colors ${
                          spec.isActive ? 'bg-green-500' : 'bg-gray-300'
                        }`}
                      >
                        <span
                          className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${
                            spec.isActive ? 'left-7' : 'left-1'
                          }`}
                        />
                      </button>
                    </div>
