...
const DoctorsSection: React.FC = () => {
  ...
  const handleToggle = (id: string) => {
    dispatch({ type: 'TOGGLE_DOCTOR', payload: id });
  };
  ...
  const getContactIcon = (type: string) => {
    switch (type) {
      ...
      case 'phone':
        return <Phone size={16} className="text-orange-500" />;
      default:
        return null;
    }
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          ...
          <p className="text-3xl font-bold text-purple-600">
            {state.doctors.filter((d) => d.videoUrl).length}
          </p>
        </div>
      </div>
        ...
        {filteredDoctors.map((doc) => (
          ...
          >
            <div className="relative">
              ...
              {doc.videoUrl && (
                <div className="absolute top-3 right-3 bg-purple-500 text-white px-2 py-1 rounded-lg text-xs flex items-center gap-1">
                  <Video size={14} />
                  Video
                </div>
              )}
                    ...
                    {(['whatsapp', 'phone', 'email'] as const).map((type) => (
                      <label
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg border cursor-pointer ${
                          formData.preferredContact === type
                      ...
                      >
                        <input
                          ...
                          value={type}
                          checked={formData.preferredContact === type}
                          onChange={() => setFormData({ ...formData, preferredContact: type })}
                          className="sr-only"
                        />
                        {getContactIcon(type)}
